...
export function createGroupCollectionInfo(
    ...
    const groupIndicesAfterSpecial = getConnectedIndices(clickedIndex, cubes).filter(
        (idx) => cubes[idx].color !== null,
    );
    ...
    return {
        ...
        specialGroupIndices,
        getGroupSizeBeforeSpecial() {
            return this.groupIndicesBeforeSpecial.length;
        },
        getGroupSizeAfterSpecial() {
            return this.groupIndicesAfterSpecial.length;
        },
        getNonSpecialGroupSize() {
            return this.nonSpecialGroupIndices.length;
        },
        getSpecialGroupSize() {
            return this.specialGroupIndices.length;
        },
    };
}
